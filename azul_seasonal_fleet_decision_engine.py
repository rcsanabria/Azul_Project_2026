...
def plot_top_volatility(route_summary, output_dir, top_n=15):
    ...
    ax.set_xlabel("Seasonal volatility score (unitless index)")
    ax.set_ylabel("Route / city pair")
    fig.tight_layout()
...
def plot_candidate_bars(recs, output_dir, recommendation_terms, filename, title, top_n=15):
    ...
    ax.set_title(title)
    ax.set_xlabel("Priority score (unitless index)")
...
def plot_top_route_profiles(route_monthly, route_summary, output_dir, top_n=5):
    ...
    for route in routes:
        ...
        ax1.plot(g["MonthStart"], g["Flights"], marker="o", label="Flights per month")
        ax1.set_xlabel("Month")
        ax1.set_ylabel("Number of flights per month")
        ax2 = ax1.twinx()
        ax2.plot(g["MonthStart"], g["AvgTOW_kg"], marker="s", linestyle="--", label="Average TOW")
        ax2.set_ylabel("Average take-off weight, TOW (kg)")
        ax1.set_title(f"Monthly Frequency and Average TOW: {route}")
        fig.autofmt_xdate()
        fig.tight_layout()
